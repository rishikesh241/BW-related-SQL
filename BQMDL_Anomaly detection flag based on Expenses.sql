WITH ExpenseData AS (
    ...
    WHERE AmountInLocalCurrency <> 0 
        ...
        AND CurrencyKey = 'USD'
        AND (
            GLParentText IN ('Gross Revenue', 'Travel Expense', 'Operating Expense', 'Cost of Goods Sold') 
            OR (GLParentText = 'Other Operating Expense' AND GLNodeText IN (
                'Bad Debt Expense', 'Advertising and Sales Costs', 'Marketing Expenses', 
                'Subcontracting Services', 'Other Operating Expenses', 'Commission/Charge (TRM)',
                'Other Expenses (TRM)', 'Cash Discounts Expense'
            ))
        )
